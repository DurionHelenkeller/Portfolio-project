...
# Write code to insert restaurant data into a data structure (LinkedList) here. The data is in data.py
...
while len(selected_food_type) == 0:
    ...
    # print list of matching_types
    for food in matching_types:
        print(food)

    # Check if only one type of restaurant was found, ask user if you would like to select this type
    if len(matching_types) == 1:
        select_type = str(input("\nThe only matching type for the specified input is " + matching_types[0] + ". \nDo you want to look at " + matching_types[0] + " restaurants? Enter y for yes and n for no\n")).lower()
        # After finding food type write code for retreiving restaurant data here
        if select_type == 'y':
